...
class GmailConnector(ConnectorPort):
    """Gmail mailboxes, messages, threads, and labels."""
    ...
    def __init__(
        ...
        default_fixture = base_dir / "fixtures" / "gmail_sample.json"
    ...
    def spec(self) -> ConnectorSpec:
        return ConnectorSpec(
            name="gmail",
            version="0.0.1",
            description="Gmail mailboxes, messages, threads, and labels",
            config_schema={
                "type": "object",
                "properties": {
                    "api_key": {"type": "string"},
    ...
    async def read(
        ...
        for item in items:
            ...
            allowed = []
            if item.get("from"):
                allowed.append(f"user:{item['from']}")
            for t in item.get("to", []):
                allowed.append(f"user:{t}")
            for c in item.get("cc", []):
                allowed.append(f"user:{c}")
