...
class GoogleDriveConnector(ConnectorPort):
    """Google Drive files, folders, shared drives, and permissions."""
    ...
    def __init__(
        ...
        default_fixture = base_dir / "fixtures" / "google_drive_sample.json"
    ...
    def spec(self) -> ConnectorSpec:
        return ConnectorSpec(
            name="google_drive",
            version="0.0.1",
            description="Google Drive files, folders, shared drives, and permissions",
            config_schema={
                "type": "object",
                "properties": {
                    "api_key": {"type": "string"},
    ...
    async def read(
        ...
        for item in items:
            ...
            for p in item.get("permissions", []):
                ptype = p.get("type")
                if ptype == "user" and "emailAddress" in p:
                    allowed.append(f"user:{p['emailAddress']}")
                elif ptype == "group" and "emailAddress" in p:
                    allowed.append(f"group:{p['emailAddress']}")
                elif ptype == "domain" and "domain" in p:
                    allowed.append(f"domain:{p['domain']}")
                elif ptype == "anyone":
                    is_public = True
                    allowed.append("user:*")
            acl = ACL(allowed_principals=allowed, is_public=is_public)

            yield Record(
                id=f"google_drive_{ext_id}",
                source="google_drive",
                external_id=ext_id,
                payload=item,
                acl=acl,
