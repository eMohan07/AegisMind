...
class TeamsConnector(ConnectorPort):
    ...
    def __init__(
        ...
        default_fixture = base_dir / "fixtures" / "teams_sample.json"
    ...
    def spec(self) -> ConnectorSpec:
        return ConnectorSpec(
            name="teams",
            version="0.0.1",
            description="Microsoft Teams, channels, and chat messages",
            config_schema={
                "type": "object",
                "properties": {
                    "api_key": {"type": "string"},
    ...
    async def read(
        ...
        for item in items:
            ...
            allowed = [f"user:{m}" for m in item.get("members", [])]
            if item.get("team_id"):
