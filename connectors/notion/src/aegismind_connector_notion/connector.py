...
class NotionConnector(ConnectorPort):
    """Notion workspaces, databases, pages, and user access lists."""
    ...
    def __init__(
        ...
        default_fixture = base_dir / "fixtures" / "notion_sample.json"
    ...
    def spec(self) -> ConnectorSpec:
        return ConnectorSpec(
            name="notion",
            version="0.0.1",
            description="Notion workspaces, databases, pages, and user access lists",
            config_schema={
                "type": "object",
                "properties": {
                    "api_key": {"type": "string"},
    ...
    async def read(
        ...
        for item in items:
            ...
            access = list(item.get("access_list", []))
            is_pub = bool(item.get("is_public", False))
            if is_pub:
                access.append("user:*")
            acl = ACL(allowed_principals=access, is_public=is_pub)
