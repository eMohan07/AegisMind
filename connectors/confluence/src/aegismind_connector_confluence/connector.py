...
class ConfluenceConnector(ConnectorPort):
    """Confluence spaces, pages, blogs, and permission restrictions."""
    ...
    def __init__(
        ...
        default_fixture = base_dir / "fixtures" / "confluence_sample.json"
    ...
    def spec(self) -> ConnectorSpec:
        return ConnectorSpec(
            name="confluence",
            version="0.0.1",
            description="Confluence spaces, pages, blogs, and permission restrictions",
            config_schema={
                "type": "object",
                "properties": {
                    "api_key": {"type": "string"},
    ...
    async def read(
        ...
        for item in items:
            ...
            users = [f"user:{u}" for u in restrictions.get("users", [])]
            groups = [f"group:{g}" for g in restrictions.get("groups", [])]
            is_public = bool(item.get("is_public", False))
            allowed = users + groups
            if is_public:
                allowed.append("group:confluence-users")
            acl = ACL(allowed_principals=allowed, is_public=is_public)

            yield Record(
                id=f"confluence_{ext_id}",
                source="confluence",
                external_id=ext_id,
                payload=item,
                acl=acl,
