...
class DropboxConnector(ConnectorPort):
    """Dropbox folders, files, and team shares."""
    ...
    def __init__(
        ...
        default_fixture = base_dir / "fixtures" / "dropbox_sample.json"
    ...
    def spec(self) -> ConnectorSpec:
        return ConnectorSpec(
            name="dropbox",
            version="0.0.1",
            description="Dropbox folders, files, and team shares",
            config_schema={
                "type": "object",
                "properties": {
                    "api_key": {"type": "string"},
    ...
    async def read(
        ...
        for item in items:
            ...
            users = [f"user:{u}" for u in item.get("shared_users", [])]
            teams = [f"group:{t}" for t in item.get("team_shares", [])]
            is_pub = bool(item.get("is_public", False))
            allowed = users + teams
            if is_pub:
                allowed.append("group:all-company")
            acl = ACL(allowed_principals=allowed, is_public=is_pub)

            yield Record(
                id=f"dropbox_{ext_id}",
                source="dropbox",
                external_id=ext_id,
                payload=item,
                acl=acl,
