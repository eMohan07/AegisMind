...
class GitHubConnector(ConnectorPort):
    """GitHub repositories, issues, pull requests, and permissions."""
    ...
    def __init__(
        ...
        default_fixture = base_dir / "fixtures" / "github_sample.json"
    ...
    def spec(self) -> ConnectorSpec:
        return ConnectorSpec(
            name="github",
            version="0.0.1",
            description="GitHub repositories, issues, pull requests, and permissions",
            config_schema={
                "type": "object",
                "properties": {
                    "api_key": {"type": "string"},
    ...
    async def read(
        ...
        for item in items:
            ...
            is_priv = bool(item.get("private", True))
            allowed = []
            if item.get("org"):
                allowed.append(f"org:{item['org']}")
            for t in item.get("teams", []):
                allowed.append(f"team:{t}")
            is_pub = not is_priv
            if is_pub:
                allowed.append("user:*")
