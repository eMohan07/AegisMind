...
class GranolaConnector(ConnectorPort):
    """Granola meeting notes, transcripts, and attendee sharing access."""
    ...
    def __init__(
        ...
        default_fixture = base_dir / "fixtures" / "granola_sample.json"
    ...
    def spec(self) -> ConnectorSpec:
        return ConnectorSpec(
            name="granola",
            version="0.0.1",
            description="Granola meeting notes, transcripts, and attendee sharing access",
            config_schema={
                "type": "object",
                "properties": {
                    "api_key": {"type": "string"},
    ...
    async def read(
        ...
        for item in items:
            ...
            allowed = [f"user:{att}" for att in item.get("attendees", [])]
            if item.get("creator") and f"user:{item['creator']}" not in allowed:
                allowed.append(f"user:{item['creator']}")
            acl = ACL(allowed_principals=allowed, is_public=False)

            yield Record(
                id=f"granola_{ext_id}",
                source="granola",
                external_id=ext_id,
                payload=item,
                acl=acl,
