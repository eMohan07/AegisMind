...
class ZendeskConnector(ConnectorPort):
    """Zendesk tickets, help center articles, and brand permissions."""
    ...
    def __init__(
        ...
        default_fixture = base_dir / "fixtures" / "zendesk_sample.json"
    ...
    def spec(self) -> ConnectorSpec:
        return ConnectorSpec(
            name="zendesk",
            version="0.0.1",
            description="Zendesk tickets, help center articles, and brand permissions",
            config_schema={
                "type": "object",
                "properties": {
                    "api_key": {"type": "string"},
    ...
    async def read(
        ...
        for item in items:
            ...
            allowed = []
            is_pub = False
            if item_type == "article" and item.get("user_segment_id") is None:
                is_pub = True
                allowed.append("user:*")
            else:
                if item.get("requester_id"):
                    allowed.append(f"user:{item['requester_id']}")
                if item.get("assignee_id"):
                    allowed.append(f"user:{item['assignee_id']}")
                if item.get("group_id"):
                    allowed.append(f"group:{item['group_id']}")
            acl = ACL(allowed_principals=allowed, is_public=is_pub)

            yield Record(
                id=f"zendesk_{ext_id}",
                source="zendesk",
                external_id=ext_id,
                payload=item,
                acl=acl,
