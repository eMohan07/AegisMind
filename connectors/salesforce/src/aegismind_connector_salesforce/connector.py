...
class SalesforceConnector(ConnectorPort):
    """Salesforce Accounts, Opportunities, Knowledge articles via SOQL."""
    ...
    def __init__(
        ...
        default_fixture = base_dir / "fixtures" / "salesforce_sample.json"
    ...
    def spec(self) -> ConnectorSpec:
        return ConnectorSpec(
            name="salesforce",
            version="0.0.1",
            description="Salesforce Accounts, Opportunities, Knowledge articles via SOQL",
            config_schema={
                "type": "object",
                "properties": {
                    "api_key": {"type": "string"},
    ...
    async def read(
        ...
        for item in items:
            ...
            allowed = []
            if item.get("OwnerId"):
                allowed.append(f"user:{item['OwnerId']}")
            if item.get("UserRoleId"):
                allowed.append(f"role:{item['UserRoleId']}")
            is_pub = bool(item.get("IsVisibleInPkb", False))
            if is_pub:
                allowed.append("user:*")
            acl = ACL(allowed_principals=allowed, is_public=is_pub)

            yield Record(
                id=f"salesforce_{ext_id}",
                source="salesforce",
                external_id=ext_id,
                payload=item,
                acl=acl,
