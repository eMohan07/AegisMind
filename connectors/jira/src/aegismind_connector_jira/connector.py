...
class JiraConnector(ConnectorPort):
    """Jira projects, issues, and security schemes."""
    ...
    def __init__(
        ...
        default_fixture = base_dir / "fixtures" / "jira_sample.json"
    ...
    def spec(self) -> ConnectorSpec:
        return ConnectorSpec(
            name="jira",
            version="0.0.1",
            description="Jira projects, issues, and security schemes",
            config_schema={
                "type": "object",
                "properties": {
                    "api_key": {"type": "string"},
    ...
    async def read(
        ...
        for item in items:
            ...
            if item.get("reporter"):
                allowed.append(f"user:{item['reporter']}")
            if item.get("assignee"):
                allowed.append(f"user:{item['assignee']}")
            for g in item.get("allowed_groups", []):
                allowed.append(f"group:{g}")
            allowed.append(f"project:{item.get('project', 'general')}")
            acl = ACL(allowed_principals=list(set(allowed)), is_public=False)
